...
int main()
{
	Mat prev_frame, curr_frame, frame, thresh_frame;
	vector<Mat> channels;
	...
	while ((char)waitKey(1) != 'q' && capture.grab())
	{
		...
		subtract(curr_frame, prev_frame, thresh_frame);
		imshow("test", thresh_frame);
		threshold(thresh_frame, thresh_frame, 10, 255, CV_THRESH_BINARY);
